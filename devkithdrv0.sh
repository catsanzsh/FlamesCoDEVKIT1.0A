...
# Installs Nintendo (GBA→Switch), *all the classic Ataris* (8‑bit/2600/5200/
# 7800/Lynx/Jaguar/ST), and PlayStation (PSX→PS4) toolchains, plus a PS5 stub.
###############################################################################
...
# Add devkitPro repo & key if missing
...
sudo apt-get install -y cc65  # cl65, ca65, etc.
...
dasm_url=https://github.com/dasm-assembler/dasm/releases/latest/download/dasm-2.20.14-linux.tar.gz
sudo wget -qO /tmp/dasm.tar.gz "$dasm_url"
...
[ ! -d jaguar-sdk ] && git clone --depth 1 https://github.com/cubanismo/jaguar-sdk.git
...
# Atari ST (68k) cross‑compiler
echo -e "${CYAN}Installing m68k‑linux‑gnu GCC for Atari ST dev…${NC}"
...
echo -e "${CYAN}Verifying devkitPro toolchains…${NC}"
...
pacman --version                   || echo -e "${RED}devkitPro pacman missing!${NC}"

# Retro & PlayStation comps
cl65 -V                             || echo -e "${RED}cc65 (8‑bit) missing!${NC}"
dasm -v | head -n1                  || echo -e "${RED}DASM (2600/7800) missing!${NC}"
m68k-linux-gnu-gcc -v               || echo -e "${RED}m68k GCC (ST) missing!${NC}"
...
# Optional: simple make test (if user is in an example dir)
if [ -f Makefile ]; then
  echo -e "${CYAN}Detected Makefile in current directory; attempting test build…${NC}"
  make || echo -e "${RED}Make test failed – investigate the toolchain setup.${NC}"
fi
